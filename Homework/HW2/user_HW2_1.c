...
//      }
        ...
        if ( (newprint == 1) && (senddone == 1) )  { // senddone is set to 1 after UART transmission is complete
            ...
            UART_printf("P1.0:%d  %d \n\r",adc_value,statevar);
...
__interrupt void ADC10_ISR(void) {
    ...
    switch(statevar){                   //There are three different situations where 
    case 1:                             //the number of the LEDs turn on vary from 0, 2, 4,
        P1OUT = 0x00;                   //due to the voltage changing adjusted by the rheostat
        if (adc_value > 256){           //case 1 correspond to voltage is less than 1/4 VCC
            statevar = 2;               //case 2 correspond to voltage is more than 1/4 VCC and less than 1/2 VCC
        }                               //case 3 correspond to voltage is more than 1/2 VCC
        break;
    case 2:
        P1OUT = 0x30;
        if (adc_value < 256){
