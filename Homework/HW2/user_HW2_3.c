...
__interrupt void ADC10_ISR(void) {
    ...
    switch(state){             //two cases, one is when turn off and when in dark for over 2s, change to case2
    case 1:                    //if in dark for less than 2s and change to in light again, stop counting time
        P1OUT = 0x00;
        if(adc_value<100){
            count++;
            if (count == 2000){
    ...
    case 2:
        P1OUT = 0xf0;          // case 2, turn on LED, if back in light, change to case1
        if(adc_value>100){
			state = 1;
		}
        break;
