...
PP        if ( (newprint == 1) && (senddone == 1) )  { // senddone is set to 1 after UART transmission is complete
...
__interrupt void TIMER1_A1_ISR (void)
{
  switch(TA1IV)
  {
      ...
      case  TA1IV_TACCR1:  // TACCR1 CCIFG  Capture interrupt
          ...
          if (previous != 0){
              edge = TA1CCTL1;
              if ((edge &= 0x08) ==  0){      //distinguish rising or falling edge
                  time1 = current - previous;  //calclualte the time period by the difference between two interrupt time
              }
              else{
                  time2 = current - previous;
              }
          }
          previous = current;  // replace  previous by current, move to next step
          break;
